...
export default function Edit({ project, options = {} }) {
    ...
    const formatIDRNoSymbol = (value) => {
        if (value === '' || value === null || value === undefined) return '0';
        return new Intl.NumberFormat('id-ID').format(value);
    };

    const [contractValueInput, setContractValueInput] = useState(project ? formatIDRNoSymbol(project.contract_value) : '0');
            ...
            <AuthenticatedLayout>
                <div className="flex items-center justify-center min-h-[60vh]">
                    <div className="text-center space-y-4">
                        <div className="size-16 bg-slate-100 dark:bg-white/5 rounded-2xl flex items-center justify-center mx-auto text-slate-400">
                            <span className="material-symbols-outlined text-3xl animate-pulse">refresh</span>
                        </div>
    ...
    const { data, setData, put, processing, errors, setError, clearErrors } = useForm({
        ...
        name: project.name || '',
        up_no: project.up_no || '',
        auction_type_id: project.auction_type_id || '',
        agency_id: project.agency_id || '',
        company_id: project.company_id || '',
        budget_type_id: project.budget_type_id || '',
        pic_id: project.pic_id || '',
        budget_year: (project.budget_year || new Date().getFullYear()).toString(),
        vendor_ids: project.vendors?.map(v => v.id) || [],
        description: project.description || '',
        
        tax_free: project.tax_free || 'Tidak',
        tax_doc: project.tax_doc || null,
        brand_origin_id: project.brand_origin_id || '',
        certification_ids: project.certifications?.map(c => c.id) || [],
        payment_term: project.payment_term || 'Tidak ada DP',
        warranty: project.warranty || 'Tidak',
        ...
        installments: project.installments?.length > 0 
            ? project.installments.map(i => ({ ...i, id: i.id || Date.now() + Math.random() })) 
            : [{ id: Date.now(), name: 'Termin 1', percentage: 0, value: 0 }]
    });
    ...
    const statusOptions = [
        { 
            ...
            icon: 'sync', 
            color: 'blue',
            description: 'Project sedang berjalan dan dalam tahap pengerjaan aktif.'
        },
        { 
            id: 'Pending', 
            icon: 'pause', 
            color: 'amber',
            description: 'Project ditangguhkan sementara atau menunggu konfirmasi lebih lanjut.'
        },
        { 
            id: 'Completed', 
            icon: 'verified', 
            color: 'emerald',
            description: 'Project telah selesai dikerjakan dan seluruh dokumen telah terpenuhi.'
        },
    ];
    ...
    return (
                ...
                <div className="hidden xl:block sticky top-0 z-40 px-4 sm:px-8 py-4 bg-slate-50/80 dark:bg-[#0b1120]/80 backdrop-blur-md border-b border-slate-200 dark:border-white/5 transition-all">
                    <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-black tracking-tight text-slate-900 dark:text-white leading-tight">Edit Project</h2>
                                <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">{project.up_no} — {project.name}</p>
                            </div>
                        </div>

                        <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-[1.25rem] w-full sm:w-auto shadow-inner border border-slate-200/50 dark:border-white/5 h-fit">
                            {statusOptions.map((opt) => (
                                <div key={opt.id} className="tooltip-container flex-1 sm:flex-none">
                                    <div className="custom-tooltip uppercase tracking-wider">{opt.description}</div>
                                    <button
                                        type="button"
                                        onClick={() => setData('status', opt.id)}
                                        className={`w-full flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] transition-all ${
                                            data.status === opt.id 
                                            ? opt.id === 'Ongoing' ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/25' :
                                              opt.id === 'Pending' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/25' :
                                              'bg-emerald-500 text-white shadow-lg shadow-emerald-500/25'
                                            : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'
                                        }`}
                                    >
                                        <span className={`material-symbols-outlined text-[16px] ${data.status === opt.id ? 'font-fill' : ''}`}>{opt.icon}</span>
                                        <span className="hidden xs:inline">{opt.id}</span>
                                    </button>
                                </div>
                            ))}
                        </div>
        ...
        >
            <div className="min-h-screen pb-32 xl:pb-20">
                ...
                `}</style>
                <Head title={`Edit Project: ${project.up_no}`} />
                ...
                {isLaunching && (
                    <div className="launch-overlay">
                        ...
                        <div className="launch-text">Saving Changes...</div>
                    </div>
                )}
                ...
                {isMobile && (
                    ...
                    >
                        <div className="flex items-center gap-3 mb-3">
                            <div className="size-9 bg-emerald-500/10 rounded-lg flex items-center justify-center shrink-0">
                                <span className="material-symbols-outlined text-emerald-600 font-black text-xl">edit_note</span>
                            </div>
                            <div className="flex flex-col min-w-0">
                                <h1 className="text-[13px] font-black text-slate-900 dark:text-white uppercase tracking-tight truncate leading-none mb-0.5">Ubah Data Proyek</h1>
                ...
                <div className="max-w-5xl mx-auto space-y-6 pt-0 xl:pt-6 pb-32 xl:pb-0">
                    <div className="glass-card rounded-none xl:rounded-[2.5rem] shadow-none xl:shadow-2xl overflow-hidden border-0 xl:border relative">
                        ...
                        <div className="p-6 sm:p-12">
                            {activeTab === 1 && (
                                <div className="space-y-8 animate-tab-content">
                                    ...
                                    {isMobile && (
                                        <div className="xl:hidden space-y-3 p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/10">
                                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1">Status Proyek</label>
                                                ...
                                                {statusOptions.map((opt) => (
                                                    <button
                                                        ...
                                                        className={`flex flex-col items-center justify-center gap-1.5 py-3 rounded-xl transition-all ${
                                                            data.status === opt.id 
                                                            ? opt.id === 'Ongoing' ? 'bg-blue-500 text-white shadow-lg' :
                                                              opt.id === 'Pending' ? 'bg-amber-500 text-white shadow-lg' :
                                                ...
                                                <div className="flex flex-wrap gap-4">
                                                    {['SKTD', 'SKB'].map(doc => (
                                                        <button 
                                                            key={doc}
                                                            type="button"
                                                            onClick={() => setData('tax_doc', doc)}
                                                            className={`flex items-center gap-4 px-8 py-5 border-2 rounded-2xl cursor-pointer transition-all group ${
                                                            data.tax_doc === doc 
                    ...
                    <div className="hidden xl:flex justify-end gap-4 px-12 pb-12">
                        ...
                        {activeTab < 3 ? (
                            ...
                            >
                                {processing ? 'Sedang Memproses...' : 'Simpan Perubahan'}
