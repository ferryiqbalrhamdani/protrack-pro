...
export default function Vendor({ vendors, filters }) {
    const { data: vendorData, links, meta } = vendors;
    ...
    const submit = (e) => {
        e.preventDefault();
        if (modalMode === 'create') {
            post(route('master.data.vendor.store'), {
                onSuccess: () => {
                    closeModal();
                },
            });
        } else {
            put(route('master.data.vendor.update', selectedVendor.id), {
                onSuccess: () => {
                    closeModal();
                },
            });
    ...
    const handleDelete = () => {
        destroy(route('master.data.vendor.destroy', selectedVendor.id), {
            onSuccess: () => {
    ...
    useEffect(() => {
        if (!isFirstRender) {
            const timeoutId = setTimeout(() => {
                router.get(route('master.data.vendor'), { search: searchQuery }, {
                    preserveState: true,
                    preserveScroll: true,
                    replace: true
                });
        ...
        <AuthenticatedLayout>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between mb-8">
                <div className="flex flex-col items-start w-full">
                    ...
                    <div className="flex justify-between items-center w-full">
                        <div>
                            <h2 className="text-2xl font-black text-slate-800 dark:text-white leading-tight">Data Vendor</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola daftar Vendor, sub-kontraktor, dan relasi penyedia barang/jasa.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div className="relative w-full sm:w-96">
                    ...
                    <input 
                        type="text" 
                        placeholder="Cari vendor atau alamat..." 
                        value={searchQuery}
                ...
                <button 
                    ...
                    className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-amber-500 text-white rounded-2xl font-bold text-sm hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 hover:-translate-y-0.5"
                >
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl shadow-slate-200/40 dark:shadow-none border border-slate-100 dark:border-white/5 overflow-hidden">
                <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 dark:bg-white/[0.02]">
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap border-b border-slate-100 dark:border-white/5">Nama Vendor</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                            {filteredVendors.map((vendor, i) => (
                                <tr key={vendor.id} className="hover:bg-slate-50/80 dark:hover:bg-white/[0.02] transition-colors group animate-slide-up-fade" style={{ animationDelay: `${i * 50 + 100}ms` }}>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-4">
                                            <div className="size-12 rounded-2xl bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600 dark:text-amber-400 shrink-0 group-hover:scale-110 group-hover:bg-amber-500 group-hover:text-white transition-all shadow-sm">
                                                <span className="material-symbols-outlined">store</span>
                                            </div>
                                            <span className="text-sm font-black text-slate-700 dark:text-slate-100 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">{vendor.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col gap-1.5">
                                            <div className="flex items-center gap-2 text-sm font-bold text-slate-600 dark:text-slate-300 bg-amber-50 dark:bg-amber-500/5 px-2.5 py-1 rounded-lg w-max border border-amber-100 dark:border-amber-500/10">
                                                <span className="material-symbols-outlined text-[16px] text-amber-500">call</span>
                                                {vendor.phone || 'N/A'}
                                            </div>
                                    ...
                                    <td className="px-8 py-6">
                                        <p className="text-sm font-medium text-slate-600 dark:text-slate-400 max-w-xs leading-relaxed truncate" title={vendor.address}>
                                            {vendor.address || 'N/A'}
                                        </p>
                                    </td>
                                    <td className="px-8 py-6">
                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-sm ${
                                            ...
                                            <span className={`size-1.5 rounded-full ${vendor.status === 'Active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`}></span>
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            <div className="relative group/tooltip">
                                                <button 
                                                    ...
                                                    className="p-2.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded-xl transition-all border border-transparent hover:border-amber-100 dark:hover:border-amber-500/20"
                                                >
                    ...
                    {filteredVendors.length === 0 && (
                        <div className="p-16 text-center flex flex-col items-center justify-center bg-slate-50/30 dark:bg-white/[0.01]">
                            ...
                            <p className="text-sm font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Vendor tidak ditemukan</p>
                        </div>
                    )}
                </div>
                ...
                <div className="p-6 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.01] flex flex-col sm:flex-row items-center justify-between gap-4">
                    <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                        Menampilkan <span className="text-slate-700 dark:text-slate-200">{vendors.from || 0}</span> hingga <span className="text-slate-700 dark:text-slate-200">{vendors.to || 0}</span> dari <span className="text-slate-700 dark:text-slate-200">{vendors.total}</span> entri
                    </span>
            ...
            <Modal show={isModalOpen} onClose={closeModal} maxWidth="3xl" premium={true}>
                <form onSubmit={submit} className="flex flex-col h-full">
                    ...
                    <div className="flex items-center justify-between p-8 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02]">
                        <div className="flex items-center gap-4">
                            <div className="size-12 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 shadow-lg shadow-amber-500/10">
                                <span className="material-symbols-outlined text-2xl font-fill">store</span>
                            </div>
                            <div>
                                <h2 className="text-xl font-black text-slate-800 dark:text-white">
                                    {modalMode === 'create' ? 'Tambah Vendor Baru' : 'Edit Data Vendor'}
                                </h2>
                    ...
                    <div className="p-8 overflow-y-auto no-scrollbar max-h-[60vh]">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2 md:col-span-2">
                                <InputLabel htmlFor="name" value="Nama Vendor" className="text-[10px] uppercase tracking-widest font-black text-slate-400" />
                                <TextInput
                                    id="name"
                                    name="name"
                                    value={data.name}
                                    ...
                                    placeholder="Contoh: PT Surya Elektronik"
                    ...
                    <div className="p-8 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] flex items-center justify-end gap-3 rounded-b-[2.5rem]">
                        ...
                        <PrimaryButton disabled={processing} className="rounded-xl px-8 py-3 bg-amber-500 hover:bg-amber-600 font-black text-[10px] uppercase tracking-widest shadow-lg shadow-amber-500/20 border-none text-white">
                            {processing ? 'Menyimpan...' : (modalMode === 'create' ? 'Simpan Vendor' : 'Perbarui Vendor')}
                        </PrimaryButton>
                    </div>
                </form>
            </Modal>
            ...
            <Modal show={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}>
                <div className="p-8 text-center bg-white dark:bg-slate-900 rounded-3xl">
                    ...
                    <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight mb-2">Hapus Vendor?</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 px-4">
                        Apakah Anda yakin ingin menghapus <strong>{selectedVendor?.name}</strong>? Tindakan ini tidak dapat dibatalkan.
                    </p>
