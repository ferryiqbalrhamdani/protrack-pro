...
export default function Company({ companies, filters }) {
    const { data: companyData, links, meta } = companies;
    ...
    const submit = (e) => {
        e.preventDefault();
        if (modalMode === 'create') {
            post(route('master.data.company.store'), {
                onSuccess: () => {
                    closeModal();
                },
            });
        } else {
            put(route('master.data.company.update', selectedCompany.id), {
                onSuccess: () => {
                    closeModal();
                },
            });
    ...
    const handleDelete = () => {
        destroy(route('master.data.company.destroy', selectedCompany.id), {
            onSuccess: () => {
    ...
    useEffect(() => {
        if (!isFirstRender) {
            const timeoutId = setTimeout(() => {
                router.get(route('master.data.company'), { search: searchQuery }, {
                    preserveState: true,
                    preserveScroll: true,
                    replace: true
                });
        ...
        <AuthenticatedLayout>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between mb-8">
                <div className="flex flex-col items-start w-full">
                    ...
                    <div className="flex justify-between items-center w-full">
                        <div>
                            <h2 className="text-2xl font-black text-slate-800 dark:text-white leading-tight">Data Perusahaan</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Kelola informasi perusahaan klien dan grup.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                ...
                <button 
                    ...
                    className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-primary text-white rounded-2xl font-bold text-sm hover:bg-primary-dark transition-all shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:-translate-y-0.5"
                >
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl shadow-slate-200/40 dark:shadow-none border border-slate-100 dark:border-white/5 overflow-hidden">
                <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 dark:bg-white/[0.02]">
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap border-b border-slate-100 dark:border-white/5">Nama Perusahaan</th>
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap border-b border-slate-100 dark:border-white/5">Kontak & Website</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                            {filteredCompanies.map((company, i) => (
                                <tr key={company.id} className="hover:bg-slate-50/80 dark:hover:bg-white/[0.02] transition-colors group animate-slide-up-fade" style={{ animationDelay: `${i * 50 + 100}ms` }}>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <span className="text-sm font-black text-slate-700 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{company.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col gap-1.5">
                                            <div className="flex items-center gap-2 text-sm font-bold text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-white/5 px-2.5 py-1 rounded-lg w-max border border-slate-100 dark:border-white/5">
                                                <span className="material-symbols-outlined text-[16px] text-slate-400">call</span>
                                                {company.phone || 'N/A'}
                                            </div>
                                            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-500 px-1">
                                    ...
                                    <td className="px-8 py-6">
                                        <p className="text-sm font-medium text-slate-600 dark:text-slate-400 max-w-xs leading-relaxed" title={company.address}>
                                            {company.address || 'N/A'}
                                        </p>
                                    </td>
                                    <td className="px-8 py-6">
                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-sm ${
                                            ...
                                            <span className={`size-1.5 rounded-full ${company.status === 'Active' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`}></span>
                    ...
                    {filteredCompanies.length === 0 && (
                        <div className="p-16 text-center flex flex-col items-center justify-center bg-slate-50/30 dark:bg-white/[0.01]">
                            ...
                            <p className="text-sm font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Perusahaan tidak ditemukan</p>
                        </div>
                    )}
                </div>
                ...
                <div className="p-6 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.01] flex flex-col sm:flex-row items-center justify-between gap-4">
                    <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                        Menampilkan <span className="text-slate-700 dark:text-slate-200">{companies.from || 0}</span> hingga <span className="text-slate-700 dark:text-slate-200">{companies.to || 0}</span> dari <span className="text-slate-700 dark:text-slate-200">{companies.total}</span> entri
                    </span>
            ...
            <Modal show={isModalOpen} onClose={closeModal} maxWidth="3xl" premium={true}>
                <form onSubmit={submit} className="flex flex-col h-full">
                    ...
                    <div className="flex items-center justify-between p-8 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02]">
                        <div className="flex items-center gap-4">
                            <div className="size-12 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-500 shadow-lg shadow-blue-500/10">
                                <span className="material-symbols-outlined text-2xl font-fill">corporate_fare</span>
                            </div>
                            <div>
                                <h2 className="text-xl font-black text-slate-800 dark:text-white">
                                    {modalMode === 'create' ? 'Tambah Perusahaan Baru' : 'Edit Data Perusahaan'}
                                </h2>
                    ...
                    <div className="p-8 overflow-y-auto no-scrollbar max-h-[60vh]">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2 md:col-span-2">
                                <InputLabel htmlFor="name" value="Nama Perusahaan" className="text-[10px] uppercase tracking-widest font-black text-slate-400" />
                                <TextInput
                                    id="name"
                                    name="name"
                                    value={data.name}
                    ...
                    <div className="p-8 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] flex items-center justify-end gap-3 rounded-b-[2.5rem]">
                        ...
                        <PrimaryButton disabled={processing} className="rounded-xl px-8 py-3 bg-primary hover:bg-primary-dark font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primary/20">
                            {processing ? 'Menyimpan...' : (modalMode === 'create' ? 'Simpan Perusahaan' : 'Perbarui Perusahaan')}
                        </PrimaryButton>
                    </div>
                </form>
            </Modal>
            ...
            <Modal show={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}>
                <div className="p-8 text-center bg-white dark:bg-slate-900 rounded-3xl">
                    ...
                    <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight mb-2">Hapus Perusahaan?</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 px-4">
                        Apakah Anda yakin ingin menghapus <strong>{selectedCompany?.name}</strong>? Tindakan ini tidak dapat dibatalkan.
                    </p>
